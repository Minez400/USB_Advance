...
// Canonical 128 KB (65536 entries) Up-case Table per exFAT specification
void ExFatFormatter::generateUpcaseTable(std::vector<uint8_t>& out_table) {
    const size_t table_size_bytes = 128 * 1024; // 131,072 bytes (65,536 entries of uint16_t)
    out_table.assign(table_size_bytes, 0);
    uint16_t* uc = reinterpret_cast<uint16_t*>(out_table.data());

    for (uint32_t i = 0; i < 65536; ++i) {
        uc[i] = static_cast<uint16_t>(i);
    }

    // Direct ASCII case conversion 0x0061 ('a') to 0x007A ('z') -> 'A' to 'Z'
    for (uint16_t ch = 'a'; ch <= 'z'; ++ch) {
        uc[ch] = ch - ('a' - 'A');
    }

    // Latin-1 Supplement case conversions
    for (uint16_t ch = 0x00E0; ch <= 0x00F6; ++ch) {
        uc[ch] = ch - 32;
    }
    for (uint16_t ch = 0x00F8; ch <= 0x00FE; ++ch) {
        uc[ch] = ch - 32;
    }
    uc[0x00FF] = 0x0178; // 'ÿ' -> 'Ÿ'

    // Cyrillic small letters 0x0430..0x044F -> 0x0410..0x042F
    for (uint16_t ch = 0x0430; ch <= 0x044F; ++ch) {
        uc[ch] = ch - 32;
    }
    uc[0x0451] = 0x0401; // 'ё' -> 'Ё'
}
...
bool ExFatFormatter::format(
    ...
    uint32_t upcase_clusters = (131072 + cluster_size - 1) / cluster_size;
    ...
    *reinterpret_cast<uint16_t*>(&vbr[106]) = 0; // Volume Flags
    ...
    if (sector_size > 512) {
        vbr[sector_size - 2] = 0x55;
        vbr[sector_size - 1] = 0xAA;
    }

    // Sectors 1 to 8: Extended Boot Sectors
    // Per Microsoft exFAT specification (Section 3.2), ExtendedBootSignature is a 4-byte value 0xAA550000
    // located at offset 2^BytesPerSectorShift - 4 (the last 4 bytes of each sector).
    for (int s = 1; s <= 8; ++s) {
        *reinterpret_cast<uint32_t*>(&boot_region[(s + 1) * sector_size - 4]) = 0xAA550000;
    }
    // Sector 9: OEM Parameter Sector (populated with GUID_NULL records, remaining bytes 0)
    // Sector 10: Reserved Sector (all zeros per exFAT specification)
    ...
    // Clean initial FAT sectors following sector 0 to remove stale residual cluster chains
    uint32_t fat_clean_sectors = std::min(fat_length_sectors - 1, 32u);
    if (fat_clean_sectors > 0) {
        std::vector<uint8_t> zero_fat(sector_size * fat_clean_sectors, 0);
        write_fn(fat_lba + 1, fat_clean_sectors, zero_fat.data());
    }
    ...
    // Compute checksum of Upcase Table per exFAT specification (all 131,072 bytes)
    uint32_t upcase_checksum = 0;
    for (size_t b = 0; b < 131072; ++b) {
    ...
    *reinterpret_cast<uint64_t*>(&u_ent[24]) = 131072; // DataLength (128 KB)
