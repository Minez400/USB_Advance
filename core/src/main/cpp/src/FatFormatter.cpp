...
bool FatFormatter::formatFat32(
    ...
    if (total_sectors >= 0x100000000ULL) {
        // FAT32 BPB_TotSec32 field is 32-bit, cannot address >= 2 TB (use exFAT instead)
        return false;
    }
    ...
    *reinterpret_cast<uint32_t*>(&vbr[67]) = volume_serial;
    ...
    // Format Volume Label (11 characters, uppercase, space-padded; default "NO NAME" per MS spec)
    ...
    *reinterpret_cast<uint32_t*>(&fsinfo[508]) = 0xAA550000; // Trail signature
    ...
    // Zero out additional FAT sectors to prevent residual fragments from prior partitions (exact allocation)
    uint32_t sectors_to_zero = std::min(sectors_per_fat - 1, 32u);
    if (sectors_to_zero > 0) {
        std::vector<uint8_t> zero_buffer(sector_size * sectors_to_zero, 0);
