...
class RootBlockDevice(
    ...
    init {
        // Safety anti-disaster validation: strictly PREVENTS access to internal system storage!
        val normalized = blockDevicePath.lowercase()
        if (normalized.contains("bootdevice") ||
            normalized.contains("userdata") ||
            normalized.contains("system") ||
            normalized.contains("vendor") ||
            normalized.contains("mmcblk0") // Internal eMMC/UFS
        ) {
            throw SecurityException("ACCESS BLOCKED: Attempted to open internal system block node: $blockDevicePath")
        }
    ...
    override suspend fun readSectors(lba: Long, count: Int, destination: ByteBuffer): Unit = withContext(Dispatchers.IO) {
        val raf = suRaf ?: throw IOException("Device closed")
        ...
        if (read < tempArray.size) {
            throw IOException("Read failure on root block device: read $read of ${tempArray.size} bytes")
        }
    ...
    override suspend fun sync(): Unit = withContext(Dispatchers.IO) {
        // "rws" mode in SuRandomAccessFile forces synchronous disk write on every operation
    }
