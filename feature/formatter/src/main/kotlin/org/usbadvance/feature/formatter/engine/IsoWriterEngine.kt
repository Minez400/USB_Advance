        ...
        try {
            ...
            if (imageSizeBytes > blockDevice.capacityBytes) {
                ...
                return@withContext IsoBurnResult(
                    ...
                    errorMessage = "A imagem ($imgMb MB) excede a capacidade do pendrive ($devMb MB)."
                )
            }
            ...
            if (blockDevice.isWriteProtected()) {
                return@withContext IsoBurnResult(
                    ...
                    durationSeconds = 0.0,
                    averageSpeedMbPerSec = 0.0,
                    errorMessage = "O dispositivo está protegido contra gravação (Write-Protect)."
                )
            }
            ...
            onProgress(
                IsoBurnProgress(
                    ...
                    stageMessage = "Iniciando gravação no pendrive..."
                )
            )
            ...
            onProgress(
                IsoBurnProgress(
                    ...
                    bytesWritten = totalWritten,
                    totalBytes = imageSizeBytes,
                    stageMessage = "Descarregando cache da memória Flash (SCSI SYNC)..."
                )
            )
            blockDevice.sync()
            ...
            onProgress(
                IsoBurnProgress(
                    ...
                    bytesWritten = totalWritten,
                    totalBytes = imageSizeBytes,
                    stageMessage = "Gravação concluída com sucesso!"
                )
            )
            ...
            IsoBurnResult(
                ...
                durationSeconds = elapsedSec,
                averageSpeedMbPerSec = 0.0,
                errorMessage = e.message ?: "Erro desconhecido durante a gravação."
            )
        }
    }
}
